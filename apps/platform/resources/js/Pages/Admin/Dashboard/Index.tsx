            ...
            <div className="dashboard-columns">
                ...
                <section className="dashboard-activity">
                    ...
                    <div className="dashboard-action-list mt-6">
                        ...
                        {hasPermission(user, 'billing.manage') ? <Link href="/admin/payment-methods" className="dashboard-action"><span className="dashboard-action__icon"><CreditCard className="size-5" aria-hidden /></span><div><strong>{ar ? 'طرق الدفع' : 'Payment methods'}</strong><p>{ar ? 'عناوين المحافظ والحسابات الظاهرة للمستأجرين.' : 'Wallet and account details shown to tenants.'}</p></div></Link> : null}
                        {hasPermission(user, 'settings.manage') ? <Link href="/admin/settings" className="dashboard-action"><span className="dashboard-action__icon"><Smartphone className="size-5" aria-hidden /></span><div><strong>{ar ? 'إعدادات المنصة والـ AI' : 'Platform & AI settings'}</strong><p>{ar ? 'تشغيل وإيقاف خدمات المنصة.' : 'Enable or disable platform services.'}</p></div></Link> : null}
